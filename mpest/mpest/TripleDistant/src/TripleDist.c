...
#	endif
	...
	for(i=1; i<nGene; i++) {
		...
		//printf("%f %d\n", tripleDist(&gtree[0], &gtree[i]), gtree[0].ntaxa - gtree[i].ntaxa);
 ...
 * printf("\n");*/
           ...
           i = 0;
           for(k=0; k<3; k++)
               i += (triplematrix1[j][k] - triplematrix2[j][k]) * (triplematrix1[j][k] - triplematrix2[j][k]);
           if(i ==0 || i == 2) dist += i; //exclude missing taxa

/* 
 * for(k=0; k<3; k++) {
 * dist += (triplematrix1[j][k] - triplematrix2[j][k]) * (triplematrix1[j][k] - triplematrix2[j][k]);
 *                         printf("%d\n", (triplematrix1[j][k] - triplematrix2[j][k]) * (triplematrix1[j][k] - triplematrix2[j][k])); 
 *
 *                                         }
 *                                         */
...
ntriples = tree2->ntaxa*(tree2->ntaxa-1)*(tree2->ntaxa-2)/6; //correction for missing taxa

return(dist / 2 / ntriples);
