...
int main (int argc, char *argv[])
{
	...
	time_t t;
	struct tm *current;
	char genetreefile[50], outfile[50], speciesname[NTAXA][50], name[50];
    ...
    /*the lengths of the external branches are 9.0, because they are unestimable*/
    for(i=0; i<sptree.ntaxa; i++) sptree.nodes[i].brlens = 9.0;
...
int Algorithm (int **triple, FILE *outfile)
{
	...
	while (round < totalround)
	{
		...
		round ++;
		if((round % sample) == 0 || stop == NUM_NOCHANGE) printf("\tround %d \t\tloglike %f \t....completed....\n",round, curLn);
		...
		if(round == 1)
    		{
				...
				fprintf(outfile, "#Nexus\n[This mpest analysis was conducted at local time %s with seed = %ld. The branch length 9.0 indicates that branch is unestimable. The tree block contains the MP-EST trees (and their likelihood scores) generated from mulitple runs specified in the control file. Choose the one with the maximum likelihood score as the estimate of the species tree.]\n", buffer, seed);		
...
void PrintHeader (void)
{
		...
		MrBayesPrint ("%s            Maximum Pseudo-likelihood Estimation of Species Trees (MPEST_1.5)  \n\n",spacer);
...
void WriteTreeToFile (Tree *tree, int inode, int showName, int showBrlens, int showTheta, int isRooted)
{
		...
		{
				if (showBrlens == YES){
                    if(showName == YES)SaveSprintf (&tempStr, &tempStrSize, "%s:9.0", tree->nodes[inode].taxaname);
    				else SaveSprintf (&tempStr, &tempStrSize, "%d:9.0", inode+1);
					AddToPrintString (tempStr);
				}else{
