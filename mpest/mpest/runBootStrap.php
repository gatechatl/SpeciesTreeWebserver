...
if ($sna_radio == "text") {
    ...
    if(move_uploaded_file($_FILES['SNAuploadfile']['tmp_name'], $original_path . "/Bootstrap/SNA.txt"))     {
        $sna = file_get_contents($original_path . "/Bootstrap/SNA.txt");
...
$file = fopen("Output/" . $uniq_id . "/Bootstrap/control", "w");
...
*/
    $output = "input.tre\n" . "0" . "\n" . "-1" . "\nReplaceMeCountGeneTrees ReplaceMeCountSpecies\n" . "ReplaceMeSNA" . "\n0" . "\n" . "";
   

} else {

    $output = "input.tre\n" . "0" . "\n" . "-1" . "\nReplaceMeCountGeneTrees " . $countspecies . "\n" . $realsna . "\n0" . "\n" . "";
}
...
#exec("ts -S 5");
exec("ts sh runMPESTBoot.sh " . $original_path);
#exec("ts sh runBootStrap.sh " . $original_path);
#exec("ts sh runDisplayBoot.sh " . $original_path);
...
<div class=\"container\">
        <div class=\"top\">
                <a>MP-EST Tree BootStrap</a><br /><br />
        </div>
